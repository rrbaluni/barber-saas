...
interface Booking { id: number; customer_name: string; service_name: string; barber_name: string; status: string; date: string; time: string; price: number; }
interface Barber { id: number; name: string; bio: string | null; active: number; }
interface Service { id: number; name: string; description: string | null; price: number; duration: number; active: number; }

function Tab({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={`px-4 py-2 text-sm font-medium rounded-lg transition ${active ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900'}`}>{label}</button>;
}
...
function BookingsTab() {
  ...
  const badge = (s: string) => {
    const map: Record<string, string> = { confirmed: 'bg-blue-100 text-blue-700', completed: 'bg-green-100 text-green-700', cancelled: 'bg-gray-100 text-gray-500', 'no-show': 'bg-red-100 text-red-700' };
      ...
      {bookings.map(b => (
        <div key={b.id} className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-start justify-between mb-2">
            <div><p className="font-semibold">{b.customer_name}</p><p className="text-sm text-gray-500">{b.service_name} with {b.barber_name}</p></div>
...
function BarbersTab() {
  ...
  const add = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name) return;
    await fetch('/api/admin/barbers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, bio }) });
    setName(''); setBio(''); load();
  };
  const toggle = async (id: number, active: number) => {
    await fetch(`/api/admin/barbers/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ active: active ? 0 : 1 }) });
    load();
  };
  return (
    <div>
      ...
      <div className="space-y-2">
        {barbers.map(b => (
          <div key={b.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-3">
            <div><p className="font-medium">{b.name}</p><p className="text-xs text-gray-500">{b.bio}</p></div>
            <button onClick={() => toggle(b.id, b.active)} className={`text-xs px-3 py-1 rounded-full font-medium ${b.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{b.active ? 'Active' : 'Inactive'}</button>
          </div>
        ))}
      </div>
    </div>
...
function ServicesTab() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    await fetch('/api/admin/services', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, description: desc, price: Number(price), duration: Number(duration) }) });
    setName(''); setDesc(''); setPrice(''); setDuration(''); load();
  };
  return (
    <div>
      ...
      <div className="space-y-2">
        {services.map(s => (
          <div key={s.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-3">
            <div><p className="font-medium">{s.name}</p><p className="text-xs text-gray-500">{s.duration}min · ${s.price}</p></div>
            <span className={`text-xs px-3 py-1 rounded-full font-medium ${s.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{s.active ? 'Active' : 'Inactive'}</span>
          </div>
        ))}
      </div>
    </div>

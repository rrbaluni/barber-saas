...
const barberServiceSeeds = [];
for (let b = 1; b <= 3; b++) {
  for (let s = 1; s <= 4; s++) {
    barberServiceSeeds.push(
      query('INSERT OR IGNORE INTO barber_services (barber_id, service_id) VALUES (?, ?)', [b, s])
    );
  }
}
await Promise.all(barberServiceSeeds);

const hoursSeeds = [];
for (let d = 0; d < 7; d++) {
  const closed = d === 0;
  hoursSeeds.push(
    query('INSERT OR IGNORE INTO business_hours (day_of_week, open_time, close_time, is_closed) VALUES (?, ?, ?, ?)',
      [d, '09:00', '18:00', closed ? 1 : 0])
  );
}
await Promise.all(hoursSeeds);

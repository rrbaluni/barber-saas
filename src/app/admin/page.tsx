  ...
  useEffect(() => {
    ...
    if (err) {
      const messages: Record<string, string> = {
        ...
        unauthorized_email: 'This email is not authorized for admin access.',
        invalid_state: 'Session expired. Please try again.',
        verification_failed: 'Google sign-in failed. Please try again.',
        missing_params: 'Invalid response from Google.',
        no_id_token: 'Could not verify identity with Google.',
      }
    ...
    return (
      <section className="min-h-screen flex items-center justify-center bg-cream dark:bg-charcoal">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="text-charcoal/60 dark:text-white/60 text-sm text-center mb-6">Sign in to manage bookings.</p>

          <div className="flex gap-2 mb-6">
            <button onClick={() => setAuthMode('password')}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${authMode === 'password' ? 'bg-charcoal dark:bg-gold text-white dark:text-charcoal' : 'bg-gold/5 text-charcoal/60 dark:text-white/60'}`}>Password</button>
            <button onClick={() => setAuthMode('google')}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${authMode === 'google' ? 'bg-charcoal dark:bg-gold text-white dark:text-charcoal' : 'bg-gold/5 text-charcoal/60 dark:text-white/60'}`}>Google</button>
          </div>
          ...
          {authMode === 'password' ? (
            ...
            <div>
              ...
              <a href="/api/auth/google"
                className="flex items-center justify-center gap-3 w-full px-6 py-3 border border-gold/10 rounded-xl text-charcoal dark:text-white font-medium hover:bg-gold/5 transition-colors">
